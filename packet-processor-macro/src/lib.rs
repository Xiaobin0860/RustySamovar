...
pub fn packet_processor(args: TokenStream, input: TokenStream) -> TokenStream {
        ...
        .map(|r| {
            match &r {
                        ...
                        && found_struct == true =>
                {
                    ...
                    stream.extend(vec![proc_macro::TokenStream::from(quote!(
                        packet_callbacks:
                            HashMap<
                                proto::PacketId,
                                fn(&mut Self, u32, &proto::PacketHead, Vec<u8>) -> (),
                            >,
                    ))]);
    ...
    let notify_handler = notify
        .clone()
        .into_iter()
        .map(|a| format!("process_{}", &a[..a.len() - 6].to_case(Case::Snake)));
    ...
    let req_handler: Vec<proc_macro2::TokenStream> =
        req_handler.map(|a| a.parse().unwrap()).collect();
    let notify_handler: Vec<proc_macro2::TokenStream> =
        notify_handler.map(|a| a.parse().unwrap()).collect();
