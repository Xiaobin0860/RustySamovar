...
impl IpcMessage {
    pub fn new_from_proto<M: prost::Message>(
        ...
        return IpcMessage(packet_id, user_id, metabuf, buf);
    }
...
impl From<Vec<u8>> for IpcMessage {
    fn from(input: Vec<u8>) -> IpcMessage {
        ...
        let data =
            input[(metadata_len + 16) as usize..(data_len + metadata_len + 16) as usize].to_owned();
