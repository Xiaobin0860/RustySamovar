...
fn get_nested_meta_name(nested_meta: &syn::NestedMeta) -> String {
    match nested_meta {
        ...
        syn::NestedMeta::Lit(_) => {
            panic!("Only identifiers are supported as an arguments to the macro")
        }
    }
}
...
pub fn excel_hash_wrapper(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let args_hash_suffix = args
        .clone()
        .into_iter()
        .map(|a| format!("{}_hash_suffix", a));
    ...
    let args_hash_pre: Vec<proc_macro2::TokenStream> =
        args_hash_pre.map(|a| a.parse().unwrap()).collect();
    let args_hash_suffix: Vec<proc_macro2::TokenStream> =
        args_hash_suffix.map(|a| a.parse().unwrap()).collect();
    let get_args_hash: Vec<proc_macro2::TokenStream> =
        get_args_hash.map(|a| a.parse().unwrap()).collect();

    let mut found_struct = false;
    let mut struct_name = None;

    let modified_struct: proc_macro::TokenStream = input
        .into_iter()
        .map(|r| {
            match &r {
                ...
                &proc_macro::TokenTree::Ident(ref ident)
                    if found_struct == true && struct_name == None =>
                {
                    // Next ident right after "struct" is the struct name
                ...
                &proc_macro::TokenTree::Group(ref group)
                    if group.delimiter() == proc_macro::Delimiter::Brace
                        && found_struct == true =>
                {
                    // Opening brackets for the struct
