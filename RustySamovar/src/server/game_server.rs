...
use rs_ipc::{IpcMessage, PushSocket, SubSocket};
    ...
    pub fn run(&mut self) {
        ...
        loop {
            let IpcMessage(packet_id, user_id, metadata, data) =
                self.packets_to_process_rx.recv().unwrap();

            if (self.login_manager.is_supported(&packet_id)) {
                self.login_manager
                    .process(user_id, packet_id, metadata, data);
            } else {
                ...
                let world = match self.worlds.entry(user_id) {
                    ...
                    Vacant(entry) => {
                        let world = GameWorld::new(
                            ...
                            self.json_manager.clone(), /*, self.packets_to_send_tx.clone()*/
                        );
                        entry.insert(world)
                    }
                };
