...
impl GameWorld {
    pub fn new(
        db: Arc<DatabaseManager>,
        jm: Arc<JsonManager>, /*, packets_to_send_tx: mpsc::Sender<IpcMessage>*/
    ) -> GameWorld {
    ...
    fn process_ping(
        ...
        req: &proto::PingReq,
        rsp: &mut proto::PingRsp,
    ) {
        rsp.client_time = req.client_time;
    }

    fn process_enter_scene_ready(
        &mut self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::EnterSceneReadyReq,
        rsp: &mut proto::EnterSceneReadyRsp,
    ) {
    ...
    fn process_scene_init_finish(
        &mut self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::SceneInitFinishReq,
        rsp: &mut proto::SceneInitFinishRsp,
    ) {
        let (current_avatar_guid, current_team_id) =
            match self.db.get_player_team_selection(user_id) {
        ...
        let avatar_enter_info = build!(AvatarEnterSceneInfo {
            ...
            avatar_entity_id: IdManager::get_entity_id_by_type_and_sub_id(
                &proto::ProtEntityType::ProtEntityAvatar,
                DatabaseManager::SPOOFED_AVATAR_ID
            ), // TODO
            avatar_ability_info: Some(build!(AbilitySyncStateInfo {})),
            weapon_guid: IdManager::get_guid_by_uid_and_id(
                user_id,
                DatabaseManager::SPOOFED_WEAPON_ID
            ), // TODO
            weapon_entity_id: IdManager::get_entity_id_by_type_and_sub_id(
                &proto::ProtEntityType::ProtEntityWeapon,
                DatabaseManager::SPOOFED_WEAPON_ID
            ), // TODO
            weapon_ability_info: Some(build!(AbilitySyncStateInfo {})),
        });
        let mp_level_info = build!(MpLevelEntityInfo {
            entity_id: IdManager::get_entity_id_by_type_and_sub_id(
                &proto::ProtEntityType::ProtEntityMpLevel,
                DatabaseManager::SPOOFED_MP_LEVEL_ID
            ), // TODO
            authority_peer_id: 1,
            ability_info: Some(build!(AbilitySyncStateInfo {})),
        });
        let team_enter_info = build!(TeamEnterSceneInfo {
            team_entity_id: IdManager::get_entity_id_by_type_and_sub_id(
                &proto::ProtEntityType::ProtEntityTeam,
                current_team_id as u32
            ),
        ...
        build_and_send!(
            ...
            PlayerEnterSceneInfoNotify {
                ...
                cur_avatar_entity_id: IdManager::get_entity_id_by_type_and_sub_id(
                    &proto::ProtEntityType::ProtEntityAvatar,
                    DatabaseManager::SPOOFED_AVATAR_ID
                ), // TODO
    ...
    fn process_enter_scene_done(
        &mut self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::EnterSceneDoneReq,
        rsp: &mut proto::EnterSceneDoneRsp,
    ) {
        rsp.enter_scene_token = req.enter_scene_token;
    ...
    fn process_post_enter_scene(
        &self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::PostEnterSceneReq,
        rsp: &mut proto::PostEnterSceneRsp,
    ) {
        let current_scene_info = match self.db.get_player_scene_info(user_id) {
            Some(scene_info) => scene_info,
            None => panic!("Scene info not found for user {}!", user_id),
    ...
    fn process_enter_world_area(
        &self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::EnterWorldAreaReq,
        rsp: &mut proto::EnterWorldAreaRsp,
    ) {
    ...
    fn spoof_scene_default_avatar(&self, user_id: u32) -> proto::SceneEntityInfo {
        let user = self
            .db
            .get_player_scene_info(user_id)
            .unwrap_or_else(|| panic!("User info not found for user {}!", user_id));
        ...
        let avatar_info = self
            .db
            .get_avatar(current_avatar_guid)
            .unwrap_or_else(|| panic!("Avatar info for avatar {} not found!", current_avatar_guid));

        let avatar_info =
            AvatarBuilder::build_avatar_info(self.jm.clone(), self.db.clone(), &avatar_info);

        let current_avatar_props = self
            .db
            .get_avatar_props(current_avatar_guid)
            .unwrap_or_else(|| panic!("Properties not found for avatar {}!", current_avatar_guid));

        let current_avatar_fight_props = self
            .db
            .get_avatar_fight_props(current_avatar_guid)
            .unwrap_or_else(|| panic!("Fight props not found for avatar {}!", current_avatar_guid));
        ...
        let authority_info = build!(EntityAuthorityInfo {
            ai_info: Some(scene_ai_info),
        });
