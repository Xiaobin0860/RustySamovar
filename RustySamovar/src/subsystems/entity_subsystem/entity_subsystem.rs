...
#[packet_processor(CombatInvocationsNotify)]
...
impl EntitySubsystem {
    ...
    fn process_combat_invocations(
        &mut self,
        user_id: u32,
        metadata: &proto::PacketHead,
        notify: &proto::CombatInvocationsNotify,
    ) {
    ...
    */
    fn handle_invoke(
        ...
        user_id: u32,
        metadata: &proto::PacketHead,
        invoke: &proto::CombatInvokeEntry,
    ) {
        match CombatTypeArgument::from_i32(invoke.argument_type).unwrap() {
            // Panics in case of unknown (undescribed in protobuf file) argument type
            CombatTypeArgument::CombatNone => self.ih_default(user_id, metadata, invoke),
            CombatTypeArgument::CombatEvtBeingHit => self.ih_being_hit(
                user_id,
                metadata,
                &EasilyUnpackable::from(&invoke.combat_data),
            ),
            CombatTypeArgument::CombatAnimatorStateChanged => {
                self.ih_default(user_id, metadata, invoke)
            }
            CombatTypeArgument::CombatFaceToDir => self.ih_face_to_dir(
                user_id,
                metadata,
                &EasilyUnpackable::from(&invoke.combat_data),
            ),
            CombatTypeArgument::CombatSetAttackTarget => self.ih_set_attack_target(
                user_id,
                metadata,
                &EasilyUnpackable::from(&invoke.combat_data),
            ),
            CombatTypeArgument::CombatRushMove => self.ih_default(user_id, metadata, invoke),
            CombatTypeArgument::CombatAnimatorParameterChanged => {
                self.ih_default(user_id, metadata, invoke)
            }
            CombatTypeArgument::EntityMove => self.ih_entity_move(
                user_id,
                metadata,
                &EasilyUnpackable::from(&invoke.combat_data),
            ),
            ...
            CombatTypeArgument::CombatCompensatePosDiff => {
                self.ih_default(user_id, metadata, invoke)
            }
    ...
    fn ih_set_attack_target(
        &self,
        user_id: u32,
        metadata: &proto::PacketHead,
        invoke: &proto::EvtSetAttackTargetInfo,
    ) {
    }
    ...
    */
    ...
    fn forward_invoke(
        ...
        match ForwardType::from_i32(invoke.forward_type).unwrap() {
            // Panics in case of unknown (undescribed in protobuf file) forward type
