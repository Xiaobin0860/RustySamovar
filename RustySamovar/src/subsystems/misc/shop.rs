...
#[packet_processor(GetShopReq, BuyGoodsReq)]
...
impl ShopSubsystem {
    ...
    fn process_get_shop(
        ...
        req: &proto::GetShopReq,
        rsp: &mut proto::GetShopRsp,
    ) {
        ...
        let shop_goods = self
            ...
            .shop_goods
            .get(&req.shop_type)
            .unwrap_or(&fuck_you_borrow_checker);
        ...
        let goods = shop_goods
            ...
            .filter_map(|item| {
                ...
                if player_level >= item.min_show_level
                    || player_level <= item.max_show_level.unwrap_or(99)
                {
                    let item_id = match item.item_id {
                        ...
                        None => match item.rotate_id {
                            ...
                            None => {
                                panic!(
                                    "Both item_id and rotate_id are empty for item {}!",
                                    item.goods_id
                                )
                            }
                        },
                    ...
                    let begin_time = match item.begin_time {
                        Some(t) => t.timestamp() as u32,
                        None => 0,
                    };
                    let end_time = match item.end_time {
                        Some(t) => t.timestamp() as u32,
                        None => 0,
                    };
                    ...
                    let good = build!(ShopGoods {
                        ...
                        goods_item: Some(build!(ItemParam {
                            item_id: item_id,
                        ...
                        cost_item_list: item
                            .cost_items
                            .iter()
                            .filter_map(|ci| if ci.item_id > 0 {
                                Some(build!(ItemParam {
                                    item_id: ci.item_id,
                                    count: ci.count,
                                }))
                            } else {
    ...
    fn process_buy_goods(
        &mut self,
        user_id: u32,
        metadata: &proto::PacketHead,
        req: &proto::BuyGoodsReq,
        rsp: &mut proto::BuyGoodsRsp,
    ) {
        ...
        self.inventory.lock().unwrap().add_item(
            user_id,
            metadata,
            goods_item.item_id,
            total_count,
            &proto::ActionReasonType::ActionReasonShop,
            true,
        );
