...
impl std::fmt::Debug for JsonManager {
    // TODO: fucking hack!
...
impl JsonManager {
    pub fn new(directory: &str) -> JsonManager {
        ...
        let reliquary_main_prop_depot: Vec<ReliquaryMainProp> =
            reader.read_json_list_game("ReliquaryMainProp");
        ...
        return JsonManager {
            ...
            shop_goods: group_nonconsec_by(shop_goods, |sg| sg.shop_type)
                .into_iter() // TODO: we're grouping by shop_type, not by goods ID!
                .collect(),
            shop_rotate: group_nonconsec_by(shop_rotate, |sr| sr.rotate_id)
                .into_iter() // TODO: we're grouping by rotate_id, not by ID!
                .collect(),
            weapons: weapons.into_iter().map(|w| (w.id, w)).collect(),
            reliquaries: reliquaries.into_iter().map(|r| (r.id, r)).collect(),

            reliquary_main_prop_depot: group_nonconsec_by(reliquary_main_prop_depot, |mp| {
                mp.prop_depot_id
            })
            .into_iter()
            .collect(), // TODO: we're grouping by depot_id!
            reliquary_affixes: group_nonconsec_by(reliquary_affixes, |a| a.depot_id)
                .into_iter()
                .collect(), // TODO: we're grouping by depot_id!

            materials: materials.into_iter().map(|m| (m.id, m)).collect(),

            teleport_points: group_nonconsec_by(teleport_points, |tp| tp.scene_id)
                .into_iter()
                .map(|(scene_id, tp_list)| {
                    (
                        scene_id,
                        tp_list.into_iter().map(|tp| (tp.point_id, tp)).collect(),
                    )
                })
                .collect(),
            ...
            proud_skills: proud_skills
                .into_iter()
                .map(|ps| (ps.proud_skill_id, ps))
                .collect(),
    ...
    pub fn roll_reliquary_stats_by_item_id(&self, item_id: u32) -> (u32, Vec<u32>) {
        let reliquary = match self.reliquaries.get(&item_id) {
            None => panic!(
                "Rolling for stats of item {} which is not in reliquary dict!",
                item_id
            ),
            Some(reliquary) => reliquary,
        };

        return self.roll_reliquary_stats(
            reliquary.main_prop_depot_id,
            reliquary.append_prop_depot_id,
            reliquary.append_prop_num,
        );
    }

    pub fn roll_reliquary_stats(
        &self,
        main_depot_id: u32,
        affix_depot_id: u32,
        num_affices: usize,
        ...
        let sub_stats: Vec<u32> = affix_depot
            .iter()
            .choose_multiple(&mut rng, num_affices)
            .iter()
            .map(|a| a.id)
            .collect(); // TODO: roll without weights!
...
impl JsonReader {
        ...
        T: DeserializeOwned,
    {
        ...
        let json_file_str =
            read_to_string(json_file_path).unwrap_or_else(|_| panic!("File {} not found", path));
        let data: Vec<T> = serde_json::from_str(&json_file_str)
            .expect(&format!("Error while reading json {}", name));
        return data;
    }
