...
use rs_ipc::{PubSocket, SubSocket};

use packet_processor::{EasilyUnpackable, PacketProcessor};
...
impl NetworkServer {
    pub fn new(host: &str, port: i16) -> Result<NetworkServer, NetworkServerError> {
        ...
        let gs = NetworkServer {
            socket: match UdpSocket::bind(format!("{}:{}", host, port).to_string()) {
                Ok(socket) => socket,
                Err(e) => {
                    return Err(NetworkServerError::new(
                        format!("Failed to bind socket: {}", e).as_str(),
                    ))
                }
            },
    ...
    pub fn run(&mut self) -> Result<i16, NetworkServerError> {
        ...
        let packet_relaying_thread = thread::spawn(move || {
            loop {
                let IpcMessage(packet_id, user_id, metadata, data) =
                    packets_to_send_rx.recv().unwrap();
                ...
                let conv = match packet_id {
                    ...
                    _ => am
                        ...
                        .unwrap()
                        .resolve_uid(user_id)
                        .unwrap_or_else(|| panic!("Unknown user ID {}!", user_id)),
                };
        ...
        loop {
            match self.socket.recv_from(&mut buffer) {
                Ok((bytes_number, source_address)) => self.process_udp_packet(
                    source_address,
                    &buffer[..bytes_number],
                    &mut auth_manager,
                ),
    ...
    fn process_udp_packet(
        ...
        packet_bytes: &[u8],
        auth_manager: &mut Arc<Mutex<AuthManager>>,
    ) {
        ...
        match hs_packet {
            Ok(hs_packet) => {
                ...
                if hs_packet.is_connect() {
                    ...
                    let mut client =
                        ClientConnection::new(self.socket.try_clone().unwrap(), conv, token);
                    client.update_source(source_address);

                    self.clients.lock().unwrap().insert(conv, client);

                    self.socket
                        .send_to(&reply.to_bytes(), source_address)
    ...
    fn process_game_packet(
        ...
        let user_id = match packet_id {
            proto::PacketId::GetPlayerTokenReq => {
                auth_manager
                    .lock()
                    .unwrap()
                    .process(conv, packet_id, data.metadata, data.data);
                return;
            }
        ...
        if packet_id == proto::PacketId::UnionCmdNotify {
            ...
            for u_cmd in union.cmd_list.into_iter() {
                self.send_packet_to_process(
                    user_id,
                    u_cmd.message_id as u16,
                    &data.metadata,
                    &u_cmd.body,
                );
            }
    ...
    fn send_packet_to_process(
        ...
        self.packets_to_process_tx.send(IpcMessage(
            packet_id,
            user_id,
            metadata.to_vec(),
            data.to_vec(),
