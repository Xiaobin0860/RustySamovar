...
#[repr(packed)]
pub struct DataPacketSmallest
// Not actually used, just represents the general structure
{
    start_magic: u16,
    packet_id: u16,
    metadata_size: u16,
...
impl DataPacket {
    ...
    pub fn new_from_bytes(raw_data: &[u8]) -> Result<DataPacket, DataDecError> {
        if raw_data.len() < std::mem::size_of::<DataPacketSmallest>() {
            return Err(DataDecError {
                reason: "Size is too small!".to_string(),
            });
        }
        ...
        if (start_magic == DataPacket::DATA_MAGIC_START)
            && (end_magic == DataPacket::DATA_MAGIC_END)
        {
            ...
            let expected_size =
                std::mem::size_of::<DataPacketSmallest>() + metadata_size + data_size;

            if raw_data.len() != expected_size {
                return Err(DataDecError {
                    reason: format!(
                        "Wrong packet size: expected {}, got {} (header {}, payload {})!",
                        expected_size,
                        raw_data.len(),
                        std::mem::size_of::<DataPacketSmallest>(),
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut ret = Vec::with_capacity(
            std::mem::size_of::<DataPacketSmallest>() + self.data.len() + self.metadata.len(),
        );
